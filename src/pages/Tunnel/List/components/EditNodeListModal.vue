    ...
    <n-modal :show="show" @update:show="$emit('update:show', $event)">
        ...
        >
            ...
            <n-flex justify="space-between" align="center">
                <n-flex>
                    <n-checkbox :checked="filters.udp" @update:checked="handleFiltersUpdate('udp', $event)">
                        UDP
                    </n-checkbox>
                </n-flex>
                <n-flex>
            ...
            <n-collapse
                ...
                @update:expanded-names="$emit('update:expandedPanels', $event)"
            >
                <n-collapse-item v-if="vipNodeCards.length > 0" name="vip" title="会员节点">
                    ...
                    <n-grid :cols="1" :x-gap="12" :y-gap="12" responsive="screen">
                        <n-grid-item v-for="(nodeCard, index) in vipNodeCards" :key="`vip-${index}`">
                            <NodeCardComponent :node="nodeCard" @click="handleNodeSelect(nodeCard)" />
                        </n-grid-item>
                    </n-grid>
                </n-collapse-item>
...
const handleNodeSelect = (nodeCard: NodeCard) => {
    emit('node-select', nodeCard.name);
